...
currentPage = page;
        fetch(`/good/data?page=${page}&size=${pageSize}`)
            .then(res => res.json())
            .then(data => {
                // Update table body
                const tbody = document.querySelector("#dataTable tbody");
                tbody.innerHTML = "";
                data.content.forEach((good, index) => {
                    tbody.innerHTML += `
                        <tr>
                            ...
                            <td>${good.category.category_name}</td>
                             <td>
                                            <a href="/good/edit/${good.id}" class="text-dark">
                                                <i class="fa-solid fa-pen-to-square"></i>
                                            </a>
                                        </td>
                                        <td>
                                            <a href="/good/delete/${good.id}" class="text-dark">
    ...
    let downloadPdfBtn = document.getElementById("downloadPdfBtn");
        ...
        downloadPdfBtn.addEventListener("click", (e) => {
                ...
                const fileName = `goodList_${Date.now()}.pdf`;
                            fetch(`/good/download/pdf?page=${currentPage}&size=${pageSize}`)
                                .then(resp => resp.blob())
                                .then(blob => {
                        ...
                        const fileName = `goodList_${Date.now()}.xlsx`;
                                                    fetch(`/good/download/excel?page=${currentPage}&size=${pageSize}`)
                                                        .then(resp => resp.blob())
                                                        .then(blob => {
                                                            const url = window.URL.createObjectURL(blob);
                                                            const a = document.createElement('a');
